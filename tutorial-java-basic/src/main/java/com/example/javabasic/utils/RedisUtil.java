...
//		redisTemplate.opsForSet().
     ...
     * @param list
     */
    public void sRefreshList(String key, Collection<String> list) {
        SetOperations<String, String> options = redisTemplate.opsForSet();
        if (StringUtils.isBlank(key)
                || CollectionUtils.isEmpty(list)) {
            return;
        }
        long size = options.size(key) == null ? 0 : options.size(key);
        if (size > 0) {
            redisTemplate.expire(key, 1, TimeUnit.NANOSECONDS);
        }

        list.forEach(e -> options.add(key, e));
    }
}
